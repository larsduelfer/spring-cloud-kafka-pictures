...
class LikeController(
    ...
    fun likeImage(imageIdentifier: UUID, authentication: Authentication): HttpResponse<LikesResource> {
        ...
        return if (authenticatedUser != null)
            HttpResponse.ok(LikesResource(imageIdentifier = imageIdentifier, likes = likeService.likeImage(imageIdentifier, authenticatedUser.id)))
        else
    ...
    fun findLikes(imageIdentifier: UUID, authentication: Authentication): HttpResponse<LikesResource> {
        return HttpResponse.ok(LikesResource(imageIdentifier = imageIdentifier, likes = likeService.findLikes(imageIdentifier)))
    }
}
